...
{ 
    ...
    if(!empty($value))
      ...
      else 
      {
        ...
        $programfile_size = ceil($_FILES['file']['size'][$key]/1048576) . " MB";
        $moviefilename=str_replace(" ", "_", $programfilename);
        
     		move_uploaded_file($source, $programsPath . $programfilename) ;
				unlink($programsPath.$_POST['oldprogramnameComplete']);
      ...
      //echo "Uploaded: program " . $destpath . $programfilename ."  ".$programfile_size. "<br/>" ;
		...
		{
		// 	$programName=$_POST['oldprogramname'];
		 	$programfilename=$_POST['oldprogramnameComplete'];
			$programfile_size=$_POST['size'];
		}
		
		$programtitle=$_POST['oldprogramname'];
    ...
    if(!empty($value))
      ...
      else 
      {
		...
		//		if($_POST['oldimagename']!="")
			//		unlink($programsCoversPath.$_POST['oldimagename']);
    ...
    if (!mysql_query("Update programs SET Name ='$programtitle', description='$_POST[description]', size='$programfile_size',  summary='$_POST[summary]', category='$newcategory', image_path='$imagefilename', program_path='$programfilename'	Where id='".$_POST['id']."'",$db))
    {
...
<title>Control Panel - Edit program</title>
...
<table width="50%" border="1" align="center" bordercolor="#FFFFFF" bgcolor="#FFFFFF" class="noneB">
  <tr>
    <td height="" colspan="3"><a href="editprograms.php" class=lang style="text-decoration:none"><b>Back</b></a></td>
  ...
  <?php 
	...
	if($added==true)  
		 					echo"<tr> <td colspan=4 ><font color=#008000>Program Updated Successfuly.</font></td></tr>";
	?>
  <tr>
    <td colspan="4" class="unnamed1"><span class="style2">Edit Program</span></td>
  </tr>
	<tr><td></td></tr>
	<tr><td></td></tr>
	<tr><td></td></tr>
...
include "include_files/paths.php";
				if(!isset($_POST['submit']))
						$q="SELECT *  FROM programs WHERE id ='". $_GET['q']."'";			
				else
						$q="SELECT *  FROM programs WHERE id ='". $_POST['id']."'";						
...
?>
 <form onsubmit="return validate_form(this)"  action="<?php echo htmlentities($_SERVER['PHP_SELF']); ?>"  method="post" enctype="multipart/form-data">
  <tr class="unnamed1">
    <td width="40%"><b>Program Title (required):</td>
		<td width=""> <input name="oldprogramname" id="oldprogramname"  type="text" value="<?php echo $r['name'];?>"</td>
		<td width="" ><font color="Orange"> <b>Change :</td>
    <td colspan="1"><input type="file" name="file[]" id="programname" /></td>
  </tr>
  <tr class="unnamed1">
    <td><b>Program Cover(Image) : </td>
		<td width=""><?php echo "<img src=$programsCoversPath".$r['image_path']." width=70 height=95 />";?> <input name="oldimagename" id="oldimagename"  type="hidden" value="<? echo $r['image_path'];?>"</td>
		<td width="" ><font color="Orange"> <b>Change :</td>
    <td colspan="1"><input type="file" name="file[]"/></td>
  </tr>										 <input name="oldprogramnameComplete" id="oldprogramnameComplete"  type="hidden" value="<?php echo $r['program_path'];?>" />
...
<!--  <tr class="unnamed1">
    ...
    <td colspan="3"><input name="programtype"   type="text" value="<?php echo $r['type'];?>" /></td>
  </tr>
-->  <tr class="unnamed1">
    <td ><b>Category :</td>
