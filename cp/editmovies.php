...
{ 
  ...
  $result=mysql_query("Select * from movies where id='".$q."'",$db);
	$row=mysql_fetch_array($result);
	unlink($moviesPath.$row['movie_path']);
//	if($row['image_path']!="")
//	  unlink($moviesCoversPath.$row['image_path']);	
	
 	mysql_query("DELETE FROM movies WHERE id='".$q."'",$db);
...
<title>Control Panel - Edit Movies</title>
...
<table width="80%" border="1" align="center" bordercolor="#FFFFFF" bgcolor="#FFFFFF" class="noneB">
  ...
  <tr>
    <td colspan="4" class="unnamed1"><span class="style2">Edit Movies</span></td>
  </tr>
	<tr><td></td></tr>
	<tr><td></td></tr>
	<tr><td></td></tr>
...
include "include_files/connect.php";
include "include_files/paths.php";
$result = mysql_query("select * from movies order by id desc",$db);
	...
	while($row = mysql_fetch_array($result))
  {
    ...
    echo '<td width=40 border=1><img src='.$moviesCoversPath.$row["image_path"] . ' width=40 height=62 border=0></img></td>';
    echo '<td border=1><span class="style4">'. $row["movie_path"] .'</span></td>';
   

     
    echo("<td align=center><a href=editmoviesfunction.php?q=".$row['id'].">Edit</a></td>"); 
		 echo("<td align=center valign=middle style=padding-top:10px><form  action=editmovies.php?q=".$row['id']. " method=post><input name=remove type=submit value=Remove   /></form></td></tr>\n\n");
	 echo '</tr>';
	} 
