...
{ 
  ...
  $result=mysql_query("Select * from programs where id='".$q."'",$db);
	$row=mysql_fetch_array($result);
	unlink($programsPath.$row['program_path']);
	
	//if($row['image_path']!="")
	  //unlink($programsCoversPath.$row['image_path']);	
	
 	mysql_query("DELETE FROM programs WHERE id='".$q."'",$db);
...
<title>Control Panel - Edit Programs</title>
...
<table width="80%" border="1" align="center" bordercolor="#FFFFFF" bgcolor="#FFFFFF" class="noneB">
  ...
  <tr>
    <td colspan="4" class="unnamed1"><span class="style2">Edit programs</span></td>
  </tr>
	<tr><td></td></tr>
	<tr><td></td></tr>
	<tr><td></td></tr>
...
include "include_files/connect.php";
include "include_files/paths.php";
$result = mysql_query("select * from programs order by id desc ",$db);
  ...
  //  echo '<td border=1><span class="style4">'. $row["type"] .'&nbsp</span></td>';
   ...
   // echo '<td border=1><span class="style4">'. $row["Required_software"] .'&nbsp</span></td>';
    echo '<td width=40 border=1><img src='.$programsCoversPath.$row["image_path"] . ' width=40 height=62 border=0></img></td>';
    echo '<td border=1><span class="style4">'. $row["program_path"] .'</span></td>';
   

     
    echo("<td align=center><a href=editprogramsfunction.php?q=".$row['id'].">Edit</a></td>"); 
		 echo("<td align=center valign=middle style=padding-top:10px><form  action=editprograms.php?q=".$row['id']. " method=post><input name=remove type=submit value=Remove   /></form></td></tr>\n\n");
	 echo '</tr>';
	} 
